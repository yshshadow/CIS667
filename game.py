...
def ai_stats(id, deck_size, player_size):
...
def node_stats(deck_size, player_size, visited_nodes):
    if (player_size == 2 and deck_size == 2):
        ai_nodes_2_2.append(visited_nodes)
    elif (player_size == 4 and deck_size == 2):
        ai_nodes_2_4.append(visited_nodes)
    elif (player_size == 3 and deck_size == 3):
        ai_nodes_3_3.append(visited_nodes)
    elif (player_size == 2 and deck_size == 1):
        ai_nodes_1_2.append(visited_nodes)
    elif (player_size == 4 and deck_size == 3):
        ai_nodes_3_4.append(visited_nodes)
    else:
        print("error")
...
class Game(object):
    ...
    def __init__(self, deck_size, player_size):
        ...
        for i in range(self.player_size):
            if (i == 0):
                player = MCST(i, self.deck, self.used_deck, self.player_size)
            else:
                player = Dummy(i)
    ...
    def score(self, id, dealer_hand, player_hand):
        if self.total(player_hand) == 21:
            ...
            if id == 0:
                ai_stats(5, self.deck_size, self.player_size)
            print(
    ...
    def game(self):
        ...
        while not all(self.stopped_count):  # stop loop until all players stop
            ...
            print("Player {}'s turn [H]it, s[W]ap, [S]top: ".format(
                current_player.id))
            action, visted_nodes = current_player.get_action()  # get action
            if (current_player.id == 0):
                node_stats(self.deck_size, self.player_size, visted_nodes)
...
if __name__ == "__main__":
    ...
    as12 = pd.DataFrame(ai_stats_1_2).transpose()
    as22 = pd.DataFrame(ai_stats_2_2).transpose()
    as24 = pd.DataFrame(ai_stats_2_4).transpose()
    as33 = pd.DataFrame(ai_stats_3_3).transpose()
    as34 = pd.DataFrame(ai_stats_3_4).transpose()
    ...
    print(statistic)
    statistic.to_csv("./data/stats_{}.csv".format(player.NN_CONFIG))

    ai_statistic = pd.concat([as12, as22, as24, as33, as34], axis=0)
    ai_statistic.columns = ['1', '2', '3', '4', '5', '6']
    ai_statistic.reset_index(drop=True, inplace=True)
    ai_statistic.index = ['1-2', '2-2', '2-4', '3-3', '3-4']
    print("AI statistic:")
    print(ai_statistic)
    ai_statistic.to_csv("./data/ai_stats_{}.csv".format(player.NN_CONFIG))

    avg_nodes_2_2 = sum(ai_nodes_2_2)/len(ai_nodes_2_2)
    avg_nodes_2_4 = sum(ai_nodes_2_4)/len(ai_nodes_2_4)
    avg_nodes_3_3 = sum(ai_nodes_3_3)/len(ai_nodes_3_3)
    avg_nodes_1_2 = sum(ai_nodes_1_2)/len(ai_nodes_1_2)
    avg_nodes_3_4 = sum(ai_nodes_3_4)/len(ai_nodes_3_4)

    avg_nodes_visited = pd.DataFrame(
        [[avg_nodes_2_2, avg_nodes_2_4, avg_nodes_3_3, avg_nodes_1_2, avg_nodes_3_4]], columns=['2_2','2_4','3_3','1_2','3_4'])
    print("Avg nodes visted for each action:")
    print(avg_nodes_visited)
    avg_nodes_visited.to_csv("./data/avg_nodes_visited_{}.csv".format(player.NN_CONFIG))
